...
const stoBaseObject = z.object({
  ...
  quantity:             z.coerce.number().int('Quantity must be a whole number').positive('Quantity must be a positive number'),
...
router.post('/', writeLimit, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    `, {
      ...
      controlled_shipping_notes:         body.controlled_shipping_required ? (body.controlled_shipping_notes ?? null) : null,
